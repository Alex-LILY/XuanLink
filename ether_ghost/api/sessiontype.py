...
_HIDDEN_SESSION_TYPES = {"REVERSE_SHELL", "LINUX_CMD_ONELINER"}
...
async def get_session_type():
    ...
    return [
        ...
        if type_id not in _HIDDEN_SESSION_TYPES
    ]
...
async def get_sessiontype_conn_options(sessiontype: str) -> list[core.OptionGroup]:
    ...
    if sessiontype in _HIDDEN_SESSION_TYPES:
        raise core.UserError("该session type已禁用")
