...
const infos = ref([])
const isPhpSession = ref(false)

onMounted(async () => {
  try {
    const session = await getDataOrPopupError(`/session/${props.session}`)
    isPhpSession.value = session && session.type && session.type.includes("PHP")
  } catch (e) {
    isPhpSession.value = false
  }
})

async function openPhpInfo() {
  addPopup("yellow", t.value.basicInfo.xssWarnTitle, t.value.basicInfo.xssWarnMsg)
...
<template>
  <div class="actions shadow-box">
    <button v-if="isPhpSession" class="action-button" @click="openPhpInfo">
      {{ t.basicInfo.openPhpInfo }}
    </button>
    <button v-if="isPhpSession" class="action-button" @click="downloadPhpInfo">
      {{ t.basicInfo.downloadPhpInfo }}
    </button>
  ...
  <div class="info-panel">
    <table class="infos shadow-box">
      <tr class="info" v-for="[i, info] in infos.entries()">
        <td class="info-data info-key" :data-tail="i != 0">{{ info.key }}</td>
        <td class="info-data info-value" :class="{ 'pre-wrap': !looksLikeHtml(info.value) }" :data-tail="i != 0">
          <span v-if="looksLikeHtml(info.value)" v-html="safeHtml(info.value)"></span>
