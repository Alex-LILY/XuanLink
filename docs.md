...
为了解决这个问题，我们需要引入简单且有效的混淆算法，将加密数据隐藏在上传图片等内容中，这就是幽魂Webshell的设计亮点。

幽魂Webshell是一种同时加密和混淆流量的一句话Webshell，在混淆流量时使用异或编码，并使用特殊的8字节标记流量的开头和结尾，从而可以将流量拼接在任意数据之中。
...
首先根据启动时打印的路径打开幽魂的配置文件夹，找到其中的`modules/php_encoders`文件夹
...
最后重启幽魂即可在webshell编辑页面看到encoder

注意：encode函数的返回值可以是str或bytes，代表需要执行的PHP代码

# 如何自定义Decoder

首先根据启动时打印的路径打开幽魂的配置文件夹，找到其中的`modules/php_decoders`文件夹
...
最后重启幽魂即可在webshell编辑页面看到decoder

# 如何将蚁剑的encoder和decoder导入到幽魂中

首先根据启动时打印的路径打开幽魂的配置文件夹

然后在`AntSwordEncoder`或`AntSwordDecoder`中写入js文件，并重启幽魂即可
...
注意：幽魂不支持和蚁剑内部设计高度相关的某些环境变量，某些encoder需要进行修改才能使用

# 如何自定义壁纸

把壁纸重命名为`bg.jpg`, `bg.png`或者`bg.webp`丢到幽魂的配置文件夹（启动时打印的路径），然后在幽魂的设置页面把主题改成“玻璃”

# Q&A: 为什么不支持在网页端添加encoder和decoder?

Encoder和Decoder会在服务端启动的时候作为代码被加载，如果蓝队登陆了红队的幽魂则可以通过添加Encoder的方式控制服务器

所以为了防止RCE漏洞，幽魂不应该支持在网页端添加encoder和decoder
